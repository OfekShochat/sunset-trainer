...
impl ResidualBlock {
  pub fn new(vs: &nn::Path, index: usize, inputs: i64, filters: i64) -> ResidualBlock {
    ResidualBlock {
      conv0: nn::conv(vs / format!("{}/conv0", index), inputs, filters, [3, 3], get_conv_config()),
      bn0: nn::batch_norm2d(vs / format!("{}/bn0", index), filters, nn::BatchNormConfig::default()),
      conv1: nn::conv(vs / format!("{}/conv1", index), inputs, filters, [3, 3], get_conv_config()),
      bn1: nn::batch_norm2d(vs / format!("{}/bn1", index), filters, nn::BatchNormConfig::default()),
    }
  }
}
...
impl ValueHead {
  pub fn new(vs: &nn::Path, inputs: i64) -> ValueHead {
    ValueHead {
      conv0: nn::conv(vs / "valuehead/conv0", inputs, 32, [1, 1], get_conv_config()),
      dense0: nn::linear(vs / "valuehead/dense0", 3200, 128, nn::LinearConfig::default()),
...
impl PolicyHead {
  pub fn new(vs: &nn::Path, inputs: i64) -> PolicyHead {
    PolicyHead {
      conv0: nn::conv(vs / "policyhead/conv0", inputs, 32, [1, 1], get_conv_config()),
      dense: nn::linear(vs / "policyhead/dense", 3200, 1859, nn::LinearConfig::default()),
    }
  }
}
...
impl NN {
  pub fn new(block_count: usize, filter_count: i64, vs: &nn::VarStore) -> NN {
    ...
    NN {
      iconv: nn::conv(&vs.root() / "iconv", 12, filter_count, [3, 3], nn::ConvConfigND::<[i64; 2]> { stride: [1, 1], padding: [1, 1], dilation: [1, 1], bias: true, groups: 1,
        ws_init: nn::Init::KaimingUniform,
        bs_init: nn::Init::Const(0.), }),
      blocks,
...
fn main() {
  ...
  let mut opt = nn::sgd(0.9, 0.0, 0.001 , true).build(&vs, config.training.lr).unwrap();
...
impl Datapoint {
  pub fn from_string(line: String) -> Option<Datapoint> {
    ...
    for s in chess::ALL_SQUARES {
      ...
      match color {
        Some(chess::Color::White) => inputs[piece.unwrap().to_index()][s.to_index() / 8][s.to_index() % 8] = 1.0,
        Some(chess::Color::Black) => inputs[piece.unwrap().to_index() + 6][s.to_index() / 8][s.to_index() % 8] = 1.0,
        None => continue,
      }
    }
